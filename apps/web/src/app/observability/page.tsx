...
export default function ObservabilityPage() {
  ...
  const hallucinationDistribution = qualityDistribution?.hallucination ?? [];
    ...
    <div className="space-y-6">
      ...
      <div className="bg-[#161b27] border border-slate-700 rounded-xl p-5">
        <h3 className="text-sm font-medium text-slate-300 mb-4">
          Hallucination Risk Trend ({hallucinationRiskTrend?.days ?? 30} days)
        </h3>
        {hallucinationRiskTrendData.length > 0 ? (
          <ResponsiveContainer width="100%" height={280}>
            <AreaChart data={hallucinationRiskTrendData}>
              <CartesianGrid stroke="#2d3748" />
              <XAxis dataKey="date" tick={{ fill: "#94a3b8", fontSize: 12 }} />
              <YAxis domain={[0, 100]} tick={{ fill: "#94a3b8", fontSize: 12 }} unit="%" />
              <Tooltip
                contentStyle={{ background: "#1e2535", border: "1px solid #374151", borderRadius: 8 }}
                formatter={(v: number) => [`${v}%`, "Avg Hallucination Risk"]}
              />
              <Legend wrapperStyle={{ color: "#94a3b8", fontSize: 13 }} />
              <Area
                type="monotone"
                dataKey="avg_hallucination_risk"
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-3">
        ...
        <div className="bg-[#161b27] border border-slate-700 rounded-xl p-5">
          <h3 className="text-sm font-medium text-slate-300 mb-4">Hallucination Risk Distribution</h3>
          {hallucinationDistribution.length > 0 && hallucinationDistribution.some((b) => b.count > 0) ? (
            <ResponsiveContainer width="100%" height={260}>
              <BarChart data={hallucinationDistribution} barCategoryGap="30%">
                <CartesianGrid stroke="#2d3748" />
                <XAxis dataKey="bucket" tick={{ fill: "#94a3b8", fontSize: 12 }} />
                <YAxis tick={{ fill: "#94a3b8", fontSize: 12 }} allowDecimals={false} />
                <Tooltip
                  contentStyle={{ background: "#1e2535", border: "1px solid #374151", borderRadius: 8 }}
                />
                <Bar dataKey="count" name="Chunks" fill="#f43f5e" radius={[4, 4, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          ) : (
            <div className="h-64 flex items-center justify-center text-slate-600 text-sm">
